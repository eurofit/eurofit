...
/**
 * Maps one formatted cart line to a GA4 ecommerce `items[]` entry. Per the GA4
 * spec `price` is the pre-discount unit price and `discount` is the per-unit
 * monetary saving, so `(price − discount) × quantity` reconciles with the line's
 * net total. `affiliation` is the storefront name and `item_variant` falls back
 * to the variant's full title when no concise variant label exists.
 */
export function toGTMCartItem(item: FormattedCartItem, index = 0): GTMCartItem {
...
/** Maps formatted cart lines to the GA4 ecommerce `items` array. */
export function toGTMCartItems(items: FormattedCartItem[]): GTMCartItem[] {
  return items.map(toGTMCartItem)
}

/**
 * Sums the net (post-discount) value of GA4 items: `Σ (price − discount) × quantity`.
 * Used as the `value` for cart ecommerce events so it reconciles with the items.
 */
export function toGTMEventValue(items: GTMCartItem[]): number {
  return items.reduce(
    (total, item) =>
